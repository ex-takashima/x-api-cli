...
export const searchAllCommand = new Command("all")
  .description("Search all posts (requires Academic/Enterprise access)")
  ...
  .action(async (query: string, options: { max: string }) => {
    ...
    try {
      ...
      const response = await client.posts.searchAll(query, {
